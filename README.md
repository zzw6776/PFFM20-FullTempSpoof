...
安装模块时不会读取或刷写 DTBO。只有在 App 中明确点击“保存并应用”或点击 Magisk Action 后，
充电 DTBO 才会单独处理：
...
2. App 先保存“期望配置”，再在外置救援目录离线读取、解包、修改和完整校验候选镜像；这个有界准备阶段绝不写 DTBO 分区。只有准备成功后才进入无 App 固定超时的短提交阶段，并在提交前重新核对配置哈希、实时分区哈希和候选哈希。准备或提交失败时配置不会倒退，可按相同配置直接重试。
3. 原始镜像、所有权状态和操作阶段以可前滚/回滚事务提交。状态格式同时记录目标补丁和上一合法补丁的哈希及配置；即使在状态提交后、分区写入前中断，也不会把自己的旧补丁误判成 OTA。救援文件保存在模块目录之外并按 `_a`/`_b` 分开，模块安装失败被 Magisk 删除时也不会连带删除备份。
4. 解包后只选择 `project-id=0x5d0d` 的子镜像，其他项目 overlay 保持不变。
5. PPS 开关开启时，写入 A16 PJZ110 官方 PPS 55W 数值和 SOC/温度曲线，并把 CPA 的 PPS 标识由 33W 改为 55W。
6. PD/QC 实验开关开启时，把已有 9V 链路的 ICL 由 2A 改为 3A、输入功率由 18W 改为 27W，
   同时只提高 PD/QC 两行 FCC 中原本 2200mA 的平台点；冷区、暖区和其他协议行不变。
7. 对 PPS 参数表和 PD/QC FCC 表逐项验证；重打包后重新检查 DT table 头、条目元数据、顺序和每个子镜像。构建器不再用零填充破坏原容器，而是逐字节保留原签名 vbmeta、按新的 DT table 长度重建 AVB footer，并复核 AVB0/vbmeta/AVBf 的位置、大小和内容。原始备份完成 `sync` 并复核后才会刷写；准备事务还会保存并校验刷写前完整镜像，刷写或回读失败时恢复该镜像和对应旧状态，而不是无条件退回最初原始 DTBO。
